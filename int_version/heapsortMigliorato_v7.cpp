...
int isheap_ric(vector<int> &a, int i, int j, int *index1, int *index2);
int isheap(vector<int> &A, int size, int *index1, int *index2);
void randomVectorGenerator(vector<int> &A, int size, int minValueOfArray, int maxValueOfArray);
void printFinalArray(vector<int> &V);
void printInputArray(vector<int> &a, int b);
...
void rearrange(vector<int> &B, int firstTreeNodeIndex, int lastTreeNodeIndex);
void createHeap(vector<int> &B);
void basicHeapsort(vector<int> &B);

int findGreaterChild(vector<int> &A, int leftChild, int j);
void heapify(vector<int> &A, int firstTreeNodeIndex, int lastTreeNodeIndex);
void buildHeap(vector<int> &A);
void moveUpGreaterChildTillHalfTree(vector<int> &A, int &currentTreeNodeIndex, int treeHeight);
void moveUpLastLeaf(vector<int> &A, int vacantNodeIndex, int lastLeaf, int vacantNodeFatherIndex);
void rebuildHeap(vector<int> &A, int currentTreeNodeIndex, int lastTreeNodeIndex, int &treeHeight);
void optimalHeapsort(vector<int> &A);

int main()
{
    int arraySize, number, minValueOfArray = 1, maxValueOfArray = 100, numberOfArraysToGenerate = 10, sumOfComparisonOptimalHeapsort = 0, sumOfComparisonBasicHeapsort = 0, dots;
    vector<int> A, B;
    double OptimalHeapsortDuration, BasicHeapsortDuration, sumOfDurationOptimalHeapsort = 0, sumOfDurationBasicHeapsort = 0;
...
bool isSorted(vector<int> &A, int *index)
{
...
void rearrange(vector<int> &B, int firstTreeNodeIndex, int lastTreeNodeIndex)
{
    int currentIndexNode = firstTreeNodeIndex, NodeToPlace = B[currentIndexNode], greaterChild;
...
void heapify(vector<int> &A, int firstTreeNodeIndex, int lastTreeNodeIndex)
{
    int greaterChild, NodeToPlace = A[firstTreeNodeIndex], leftChild, rightChild;
    bool flag = true;
...
void optimalHeapsort(vector<int> &A)
{
    int temp, firstTreeNodeIndex, treeHeight, lastTreeNodeIndex;
